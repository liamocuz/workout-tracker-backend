...
CREATE TABLE IF NOT EXISTS workout_template_strength_exercise_value (
    id BIGSERIAL PRIMARY KEY,
    weight REAL NOT NULL CHECK ( weight > 0 ),
    sets INT NOT NULL CHECK ( sets > 0 ),
    reps INT NOT NULL CHECK ( reps > 0 ),
...
CREATE INDEX IF NOT EXISTS workout_template_strength_exercise_value_seid_idx
    ON workout_template_strength_exercise_value(strength_exercise_id);
...
CREATE TABLE IF NOT EXISTS workout_template_join_strength_exercise_value (
    workout_template_id BIGINT NOT NULL REFERENCES workout_template(id) ON DELETE CASCADE,
    strength_exercise_value_id BIGINT NOT NULL REFERENCES workout_template_strength_exercise_value(id),
    PRIMARY KEY (workout_template_id, strength_exercise_value_id)
);
